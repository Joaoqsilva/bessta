...
export const SettingsPage = () => {
    ...
    const { store, logout, user } = useAuth();
    ...
    // Load current plan from backend on mount
    useEffect(() => {
        const loadSubscriptionStatus = async () => {
            try {
                ...
                // Fallback to user plan from context if API fails
                if ((user as any)?.plan) {
                    setCurrentPlan((user as any).plan);
                }
            }
        };
    ...
    const copyStoreLink = () => {
        navigator.clipboard.writeText(`https://bookme.app/store/${storeData.slug}`);
        alert('Link copiado!');
    };
                            ...
                            <div className="domain-card" style={{
                                ...
                                <div style={{
                                    ...
                                    <code style={{
                                        ...
                                        bookme.app/store/{storeData.slug || 'sua-loja'}
                                    </code>
                                    <Button
                                        variant="outline"
                                        size="sm"
                            ...
                            <div style={{
                                ...
                                <div>
                                    ...
                                    <p style={{ fontSize: '0.85rem', color: 'var(--info-700)', margin: 0 }}>
                                        ...
                                        em vez de bookme.app/store/sua-loja. Isso transmite mais profissionalismo e fortalece sua marca.
                                    </p>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="store-link-card" style={{ marginTop: '1.5rem' }}>
                                    <div className="store-link-info">
                                        ...
                                        <a href={`/store/${storeData.slug}`} className="store-link" target="_blank">
                                            bookme.app/store/{storeData.slug}
