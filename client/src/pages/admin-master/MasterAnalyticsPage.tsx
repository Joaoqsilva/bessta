...
import { getAllRegisteredStores, getPlatformStats, type RegisteredStore } from '../../context/AdminMasterService';
...
export const MasterAnalyticsPage = () => {
    ...
    const loadData = () => {
        ...
        // Calculate top stores by appointments
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            allStores.forEach(store => {
                ...
                appointments.forEach(apt => {
                    ...
                    if (aptDate.getMonth() === date.getMonth() && aptDate.getFullYear() === date.getFullYear()) {
                        monthAppointments++;
                        if (apt.status === 'completed') {
                            monthRevenue += apt.servicePrice || 0;
                        }
                    }
                });
            });

            const newStoresThisMonth = allStores.filter(s => {
                const storeDate = new Date(s.createdAt);
                return storeDate.getMonth() === date.getMonth() && storeDate.getFullYear() === date.getFullYear();
            }).length;
